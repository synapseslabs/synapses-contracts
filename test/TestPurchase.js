...
contract('Purchase', accounts => {
  ...
  beforeEach(async function() {
    // Publication that we will be buying
    publicationInstance = await UnitPublication.new(
      seller,
      ipfsHash,
      price,
      unitsAvailable,
    ...
    instance = await Purchase.new(publicationInstance.address, 0, ipfsHash, buyer, {
      from: buyer
    })
  })
...
contract('Purchase', accounts => {
  ...
  let purchase
  let publication
  let fractionalPublication
  const totalPrice = 48
  const initialPayment = 6

  describe('Success path flow: unit publication', async () => {
    before(async () => {
      publication = await UnitPublication.new(
        seller,
        ipfsHash,
        totalPrice,
        unitsAvailable,
    ...
    it('should create and link the new purchase', async () => {
      ...
      purchase = await Purchase.at(publicationPurchasedEvent.args._purchaseContract)

      assert.equal(await publication.getPurchase(0), purchase.address)
      assert.equal(await purchase.publicationContract(), publication.address)
  ...
  describe('Success path flow: fractional publication', async () => {
    before(async () => {
      fractionalPublication = await FractionalPublication.new(seller, ipfsHash, {
        from: seller
      })
    })

    it('should create and link the new purchase', async () => {
      const tx = await fractionalPublication.request(ipfsHash, {
        from: buyer,
        value: initialPayment
      })
      ...
      assert.equal((await purchase.publicationVersion()).toNumber(), 0)
    })
  ...
  describe('Buyer timeout', async () => {
    beforeEach(async () => {
      ...
      const buyTransaction = await publication.buyPublication(1, {
        from: buyer,
  ...
  it('should not allow purchase of publication by its seller', async () => {
    publication = await UnitPublication.new(
      seller,
      ipfsHash,
      totalPrice,
    ...
    try {
      await publication.buyPublication(1, {
        from: seller,
        value: totalPrice
      })
      assert.ok(false, 'allowed a seller to purchase their own publication')
    } catch (err) {
      assert.ok(isEVMError(err), 'an EVM error should be thrown')
    }
  })
